...
def plot_image_with_arrows(
    ...
    for x_start, x_end, y_frac, label, txt_off in zip(
        x_starts, x_ends, arrow_ys, labels, text_offsets, strict=False
    ):
...
def plot_image_with_arrow_and_circles(
    ...
    plt.annotate(
        ...
        xytext=(tail_x, y_pos),
        arrowprops=dict(
            facecolor=arrow_color, edgecolor=arrow_color, arrowstyle="->", linewidth=arrow_linewidth
        ),
    )
...
def load_name_conversion(name_conv_dict):
    name_dict = {}
    rev_dict = {}
    name_conv_dict = Path(name_conv_dict)
...
def generate_node_consensus(tree, node_num, alignment_file, name_conv_dict, gap_dict_json):
    ...
    gap_dict_json = Path(gap_dict_json)
    with open(gap_dict_json) as f:
...
def plot_evo_path_quiver(scores_df, tree, leaf_name, legend_loc = None):
    ...
    plt.scatter(x_vals, y_old, color="black", marker="o", label="ML ancestor", s=80)

    # Upward arrows in blue
    plt.quiver(
        x_vals[up_indices],
        y_old[up_indices],
        dx[up_indices],
    ...
    plt.scatter(
        x_vals, y_new, color="black", marker="X", label="Consensus ancestor", alpha=0.8, s=80
    )
...
def plot_multiple_evo_lines(score_dfs_leaves_labels, tree, normalize=True):
    ...
    for i, (scores_df, leaf_name, label_name) in enumerate(score_dfs_leaves_labels):
        ...
        # Get leaf value
        leaf_value = scores_df[scores_df["sequence_id"] == leaf_name][
            "pseudo_perplexity"
        ].to_list()[0]
        ...
        # Main line
        plt.plot(for_plot["bl_to_root"], for_plot["plot_value"], color=color, label=label_name)
...
def violin_plot(orig_input_df, bins, bin_labels):
    ...
    input_df["ML_prob_bin"] = pd.cut(
        input_df["ML prob"], bins=bins, labels=bin_labels, include_lowest=True
    )
    ...
    group_stats = (
        input_df.groupby("ML_prob_bin", observed=False)["pseudo_perplexity"].agg("median").values
    )
...
def kruskal_wallis_with_significant_posthoc(data_df, bins, bin_labels):
    ...
    data_df["ML_prob_bin"] = pd.cut(
        data_df["ML prob"], bins=bins, labels=bin_labels, include_lowest=True
    )
    ...
    groups = [
        group["pseudo_perplexity"].values
        for _, group in data_df.groupby("ML_prob_bin", observed=False)
    ]
    ...
    posthoc = sp.posthoc_dunn(
        data_df, val_col="pseudo_perplexity", group_col="ML_prob_bin", p_adjust="bonferroni"
    )
